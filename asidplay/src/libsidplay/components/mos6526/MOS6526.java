...
public abstract class MOS6526 {
	...
	protected abstract class Timer extends Event {
		...
		protected static final int CIAT_CR_MASK    = 0x039;

		protected static final int CIAT_CR_START   = 0x001;
		protected static final int CIAT_COUNT2     = 0x002;
		protected static final int CIAT_STEP       = 0x004;
		protected static final int CIAT_CR_ONESHOT = 0x008;
		protected static final int CIAT_CR_FLOAD   = 0x010;
		protected static final int CIAT_PHI2IN     = 0x020;
		protected static final int CIAT_COUNT3     = 0x040;
		protected static final int CIAT_LOAD1      = 0x080;
		protected static final int CIAT_LOAD       = 0x100;
		protected static final int CIAT_ONESHOT0   = 0x200;
		protected static final int CIAT_ONESHOT    = 0x400;
		protected static final int CIAT_OUT        = 0x800;
		...
		public void clock() {
			...
			if ((state & CIAT_CR_FLOAD) != 0) {
				adj |= CIAT_LOAD1;
			}
			if ((state & CIAT_CR_ONESHOT) != 0) {
				adj |= CIAT_ONESHOT0;
			}
			/* LOAD1 -> LOAD, ONESHOT0 -> ONESHOT */
			adj |= (state & (CIAT_LOAD1 | CIAT_ONESHOT0)) << 1;
			state = adj;
	...
	private final class TimerA extends Timer {
		...
		public void underFlow() {
			interruptSource.trigger((byte) 1);
	...
	private final class TimerB extends Timer {
		...
		@Override
		public void underFlow() {
			interruptSource.trigger((byte) 2);
		}
	}
	...
	protected int m_todCycles, m_todPeriod;
	 ...
	 */
	protected MOS6526(final EventScheduler ctx) {
		this.context = ctx;
		this.interruptSource = new InterruptSource6526A();
		a = new TimerA();
		b = new TimerB();
		reset();
	}
	 ...
	 *            value to write
	 */
	public void write(final int addr, byte data) {
		a.syncWithCpu();
		b.syncWithCpu();
		...
		case CRB: {
			...
			final boolean start = (data & 1) != 0 && (regs[addr] & 1) == 0;
			if (start) {
