 ...
 * 
 * For a good German documentation on the MOS 6567/6569 Videocontroller (VIC-II), see <a
 * href='http://www.minet.uni-jena.de/~andreasg/c64/vic_artikel/vic_artikel_1.htm'>http://www.minet.uni-jena.de/~andreasg/c64/vic_artikel/vic_artikel_1.htm</a> , <a
 * href='http://cbmmuseum.kuto.de/zusatz_6569_vic2.html'>http://cbmmuseum.kuto.de/zusatz_6569_vic2.html</a> or <a
 * href='http://unusedino.de/ec64/technical/misc/vic656x/vic656x-german.html'>http://unusedino.de/ec64/technical/misc/vic656x/vic656x-german.html</a>. An English version of the documentation you can
 * find at <a href='http://www.unusedino.de/ec64/technical/misc/vic656x/vic656x.html'>http://www.unusedino.de/ec64/technical/misc/vic656x/vic656x.html</a> .<br>
 * 
...
public abstract class VIC extends Bank {
	...
	/** Output RGBA screen buffer as int32 array */
	...
	/**
	 * Get the video memory base address, which is determined by the inverted bits 0-1 of port A on CIA 2, plus the video matrix base address plus the character data base address plus the bitmap
	 * memory base address.
	 */
	...
	/* This table can be used to quadruple every bit, for instance
	 * %1011 -> %1111000011111111. Additionally, the inverse of
	 * input is stored on the high pits, making the complete value
	 * %00001111000000001111000011111111. */
	...
	protected void drawSpritesAndGraphics() {
		...
		/* This should happen on the 6th, 7th or such pixel.
		 * It's apparently related to the fall time in NMOS, and
		 * can be observed to change with system temperature. */
		videoModeColorDecoderOffset &= ((registers[0x11] & 0x60 | registers[0x16] & 0x10) >> 2);
		...
		while (current != null) {
			...
			/* Handle sprite-sprite collision. It's easy to detect that
			 * a collision occurred, but hard to find which two sprites collided.
			 * For this purpose, we store the sprite index which contributed
			 * a pixel into the low bits of a pixel slot, and keep the 4th bit
			 * as a mask that we can use to reliably identify the collision.
			 */
			if ((opaqueSpritePixels & spriteForegroundMask) != 0) {
				...
				for (int pixel = 0; pixel < 32; pixel += 4) {
					...
					if (otherSprite == 0) {
						 ...
						 * but keep the old value, as it doesn't matter
						 * whether it contains the other or ourselves: both
						 * collision bits are set on 0x1e once we finish. */
		...
		for (int j = 0; j < 2; j ++) {
			...
			for (int i = 0; i < 4; i ++) {
				...
				pixels[nextPixel++] = combinedLinesCurrent[lineColor & 0xff | previousLineColor << 8 & 0xff00];
	 ...
	 * It is appropriate to use outside renderable screen, where
	 * graphics sequencer is known to have quiesced.
	 */
	 ...
	 * VIC reads on every cycle, and this describes what it reads.
	 */
	...
	public final void write(int register, final byte data) {
		...
		case 0x11:
			 ...
			 * for that line -- no more PHI1 cycles with that rasterY value remain. */
			if (lineCycle != 9) {
	...
	private void handleSpriteVisibilityEvent(final Sprite sprite) {
		 ...
		 * 
		 * VICE test spritex proves that setting location very
		 * near will not work: it takes one cycle for VIC to
		 * adjust to the new sprite position.
		 */
		if (count <= 0) {
