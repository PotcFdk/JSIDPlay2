...
class PSid extends Prg {
	...
	private static class PHeader {
		 ...
		 * For version 0x0003, all 32 chars can be used without zero termination.
		 * if less than 32 chars are used then it should be terminated with a trailing zero
		 */
		...
		/**
		 * only version 0x0002+
		 * reserved for version 0x0002
		 * used in version 0x0003 to indicate second SID chip address
		 */
	...
	protected PSid() {
		// Reloc65 modifies the driver code, for that reason a copy of IPSIDDrv.PSIDDRV should used instead
		// Otherwise A parallel searchIndexerThread can happen to fail!
		System.arraycopy(IPSIDDrv.PSIDDRV, 0, driver, 0, IPSIDDrv.PSIDDRV.length);
	}
	...
	protected void findPlaceForDriver() throws SidTuneError {
		...
		relocatedBuffer = relocator.reloc65(driver, driver.length, info.determinedDriverAddr - 10, new HashMap<String, Integer>());
	...
	protected static final SidTune load(final byte[] dataBuf) throws SidTuneError {
		...
		if (Arrays.equals(pHeader.id, new byte[] { 'P','S','I','D' })) {
			...
			sidtune.info.compatibility = (pHeader.flags & PSID_BASIC) != 0 ? Compatibility.RSID_BASIC : Compatibility.RSID;
		...
		sidtune.info.infoString[0] = makeString(pHeader.name, 0, Math.min(i, pHeader.name.length));
		...
		sidtune.info.infoString[1] = makeString(pHeader.author, 0, Math.min(i, pHeader.author.length));
		...
		sidtune.info.infoString[2] = makeString(pHeader.released, 0, Math.min(i, pHeader.released.length));
	...
	@Override
	public void save(final String name, final boolean overWrite) throws IOException {
