 ...
 * <li>Data (if any)
 * </ul>
 * <li>All commands are ACKed with a response packet that takes one of the
 * following forms:
 * <ul>
 ...
 * <li>READ: successful read operation, one byte value follows that is the value
 * read from SID.
 ...
 * <li>INFO: info packet, which contains model code and zero-padded 20-byte
 ...
 * <ul>
 * <li>data packet must be a 8-bit unsigned value which is written to volume
 * register.
 * <li>returns BUSY until audio quiescent, otherwise OK.
 * </ul>
 *
 * <li>TRY_DELAY (4): inform emulation that no events have occurred for a given
 * count of cycles
 * <ul>
 * <li>data packet must be 16-bit unsigned value interpreted as delay in C64
 ...
 * <ul>
 * <li>16-bit unsigned value interpreted as delay before the write in C64
 * clocks.
 ...
 * <ul>
 * <li>data packet must be a 4n+3 bytes long, where n >= 0. The protocol used
 ...
 * <li>8-bit unsigned SID register number from 0x00 to 0x1f.
 * </ul>
 * <li>returns BUSY if there is already enough data for playback, otherwise READ
 ...
 * <li>sid number is ignored.
 * <li>data packet must be 0 length.
 * <li>returns 2 bytes: VERSION and a data byte, which is the version of the SID
 * Network protocol.
 * </ul>
 * 
 ...
 * <li>returns BUSY until audio quiescent, otherwise OK.
 * 
 * <li>GET_CONFIG_COUNT (10): Query number of SID configurations supported by
 * server.
 * <li>sid number is ignored.</li>
 * <li>data packet is ignored and should be 0 length.
 * <ul>
 * <li>always returns COUNT and a 8-bit unsigned value that is 1 larger than the
 ...
 * <ul>
 * <li>data packet is ignored and should be 0 length.
 * <li>returns INFO and 8-bit unsigned-value and a string in ISO-8859-1 encoding
 ...
 * NOTES
 * <p>
 * The delay values do not contain the time taken to write the value to SID
 * chip, and a delay length of 0 between writes is impossible to achieve with a
 * true C64 system, although this emulator will accept it and execute several
 * writes on the same clock.
 * <p>
 * At start of connection, the SID starts from RESET state with volume=0 and
 * empty buffer.
 * <p>
 * Suitable packet size for TRY_WRITE is about 20 ms long. If TRY_WRITE returns
 * BUSY, then client should wait about 20 ms (same as the play length of one
 * packet) before retry.
 * <p>
 * Future expansion:
 * <ul>
...
public class NetworkSIDDevice extends Application {
	...
	protected static SIDChip getSidConfig(int sidNumber) {
		...
		if (iniFilter.isReSIDFilter6581()) {
			...
			sid = new residfp_builder.resid.SID();
			((residfp_builder.resid.SID) sid).setChipModel(ChipModel.MOS6581);
			Filter6581 filter6581 = ((residfp_builder.resid.SID) sid)
					.getFilter6581();
			...
			sid = new residfp_builder.resid.SID();
			((residfp_builder.resid.SID) sid).setChipModel(ChipModel.MOS8580);
			Filter8580 filter8580 = ((residfp_builder.resid.SID) sid)
					.getFilter8580();
