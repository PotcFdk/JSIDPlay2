...
public final class EnvelopeGenerator {
	...
	/**
	 * During release mode, the SID approximates envelope decay via piecewise linear
	 * decay rate.
	 */
	 ...
	 * used (decay or release), one extra cycle is spent before the envelope is
	...
	protected void clock() {
		...
		if (state == State.ATTACK || ++exponential_counter == exponential_counter_period) {
			...
			case ATTACK:
				...
				// state to
				// release, then to attack. The envelope counter is then frozen
				// at
	...
	private void set_exponential_counter() {
		...
		case 0x00:
			...
			// E.g. set R = 0xf, gate on to 0x06, gate off to 0x00, gate on to
			// 0x04,
			// gate off, sample.
			exponential_counter_period = 1;
	...
	protected void writeCONTROL_REG(final byte control) {
		...
		if (!gate && gate_next) {
			...
			hold_zero = false;
			// FIXME: This is an assumption which should be checked using cycle
			// exact
			// envelope sampling.
			envelope_pipeline = false;
		}
