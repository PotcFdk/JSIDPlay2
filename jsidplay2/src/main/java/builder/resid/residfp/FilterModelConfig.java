...
public final class FilterModelConfig {
	...
	public static double estimateFrequency(final IFilterSection filter, final int fc) {
		if (filter.getK() == 0f) {
			...
			final double R = filter.getBaseresistance() * dynamic / (filter.getBaseresistance() + dynamic);
