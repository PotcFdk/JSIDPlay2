...
public final class EnvelopeGenerator {
	/**
	 * The envelope state machine's distinct states. In addition to this, envelope
	 * has a hold mode, which freezes envelope counter to zero.
	 */
	...
	/**
	 * XOR shift register emulated via normal integer which implements delay until
	 * the next envelope operation occurs. The XOR shift register has 0x7fff
	 * different values to scan.
	 */
	 ...
	 * periods.
	 */
	private static final int[] ENVELOPE_PERIOD = { 8, // 2ms*1.0MHz/256 = 7.81
	...
	/**
	 * @param attack_decay attack/decay value
	 */
	...
	/**
	 * @param sustain_release sustain/release value
	 */
	...
	/**
	 * When CPU updates rate period, check for ADSR delay bug before allowing the
	 * update to proceed.
	 *
	...
	private void cpuUpdateRatePeriod(final int newRateCounterPeriod) {
		/*
		 * Handle edge case: cpu writing the same value that is already set in the
		 * register. This can't cause a ADSR delay bug.
		 */
		...
		/*
		 * The ADSR counter is XOR shift register with 0x7fff unique values. If the
		 * rate_period is adjusted to a value already seen in this cycle, the register
		 * will wrap around. This is known as the ADSR delay bug.
		 *
		 * To simplify the hot path calculation, we simulate this through observing that
		 * we add the 0x7fff cycle delay by changing the rate_counter variable directly.
		 * This takes care of the 99 % common case. However, playroutine could make
		 * multiple consecutive rate_period adjustments, in which case we need to cancel
		 ...
		 * cycle. This consideration is reflected by the > rather than >= and <= rather
		 * than < in the two comparisons above.
		 */
	}
}
