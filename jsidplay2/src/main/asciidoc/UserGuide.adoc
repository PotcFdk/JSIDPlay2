...
3.
Additionally we support SIDBlaster USB devices, e.g. link:http://crazy-midi.de/joomla/index.php/home/myprojects/elctronicp/58-sidblaster[SIDBlaster device TicTac Edition]. It contains exactly one chip either MOS6581 or MOS8580 per device.

If you need information to use that device, have a look here:
...
It requires ftd2xx library on the target machine, therefore do the same documented in section "Installing the required ftd2xx library" for SIDBlaster to install it:
https://haendel.ddns.net/~ken/sidblaster.html
...
link:https://sourceforge.net/p/jsidplay2/code/HEAD/tree/trunk/jsidplay2/[HardSID4U driver and signing instructions is explained in README]
