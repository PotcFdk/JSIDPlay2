...
unaccepted by iPhones and mobiles running Android9!

==== Use trusted certificate
*Info*: This example is related to Let’s Encrypt and Apache2 installation on Linux.

If your web-server and JSIDPlay2 server is both in the same domain, then you can use HTTPS using the same certificate for both: your web-server and JSIDPlay2 AppServer.
...
Let's Encrypt already adds the certbot script to renew certificates regularly.
...
Return a mp4 stream of the specified demo. On the server side the emulator is started and streams the sound output back to the client.
All parameters are used to specify emulation settings that should be used.
