...
=== Launch JSIDPlay2 AppServer with WhatsSid? tune recognition
...
*Note:* To make use of WhatsSid? tune recognition you must first create the database!
...
To start the WhatsSid database creator containing the tune fingerprintings:
[source,subs="attributes+"]
----
#!/bin/bash -x
cd /home/ken/Downloads/{artifactId}-{version}
...
*Note:* The pathToMusicCollection is traversed recursively and every song is converted to a KHz WAV recording. You will need a lot of disk space to store them (around hundreds of MB)!
