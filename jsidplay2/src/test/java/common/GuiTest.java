...
public abstract class GuiTest {
	...
	/**
	 * Creates and displays a new stage, using {@link getRootNode()} as the root
	 * node.
	 */
	...
	private static Set<Node> findAll(String query, Object parent) {
		try {
			...
			// Ignore, something went wrong with checking a window, so return an
			// empty set.
		}
	...
	private static <T extends Node> Set<T> getVisibleNodes(Set<T> foundNodes) {
		...
		if (visibleNodes.isEmpty())
			throw new NoNodesVisibleException(
					"Matching nodes were found, but none of them were visible. Screenshot saved as "
	...
	/**
	 * Returns a Callable that calculates the number of nodes that matches the
	 * given query.
	 *
	 * @param nodeQuery
	 *            a CSS query or the label of a node.
	 * @return
	 */
	 ...
	 *
	 * @param query
	 *            either CSS selector, label of node or class name.
	 * @param buttons
	 */
	 ...
	 *
	 * @param buttons
	 *            defaults to the primary mouse button.
	 */
	 ...
	 *
	 * @param amount
	 *            the number of notches to scroll
	 * @param direction
	 * @return
	 */
	...
	/**
	 * Types the given text on the keyboard. Note: Typing depends on the
	...
	/**
	 * Presses and releases a given key. If multiple keys are passed to this
	 * method, they will be released in the inverse order, to allow for pushing
	 * key combinations like this:<br>
	 * 
	 * <pre>
	...
	public GuiTest push(KeyCode... keys) {
		...
		// If waiting turns out to be necessary, put something like
