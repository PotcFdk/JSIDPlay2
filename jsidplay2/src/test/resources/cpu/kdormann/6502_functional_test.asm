...
;report.asm as I/O channel, add 3.5 kB)
report = 1
rep_int = 0
...
;masking of always on bits after PHP or BRK (unused & break) on compare
        !if I_flag = 0 {
!macro load_flag arg1x  {
            lda # arg1x & m8i         ;force enable interrupts (mask I)
            }
!macro cmp_flag arg1x     {
            cmp #( arg1x | fao)&m8i   ;I_flag is always enabled + always on bits
            }
!macro eor_flag arg1x     {
            eor #( arg1x & m8i|fao)   ;mask I, invert expected flags + always on bits
            }
        }
        !if I_flag = 1 {
!macro load_flag arg1x    {
            lda # arg1x |intdis      ;force disable interrupts
            }
!macro cmp_flag arg1x     {
            cmp #( arg1x |fai)&m8    ;I_flag is always disabled + always on bits
            }
!macro eor_flag arg1x     {
            eor #( arg1x |fai)       ;invert expected flags + always on bits + I
            }
        }
        !if I_flag = 2 {
!macro load_flag arg1x    {
...
!macro cmp_flag arg1x     {
            ...
            cmp #( arg1x |fao)&m8i   ;expected flags + always on bits, mask I
            }
!macro eor_flag arg1x     {
            eor flag_I_on       ;I_flag is never changed
            eor #( arg1x &m8i|fao)   ;mask I, invert expected flags + always on bits
        ...
        !if I_flag = 3 {
!macro load_flag arg1x {
            lda #arg1x             ;allow test to change I-flag (no mask)
            }
!macro cmp_flag arg1x    {
            cmp #( arg1x |fao)&m8    ;expected flags + always on bits
            }
!macro eor_flag arg1x     {
            eor # arg1x |fao         ;invert expected flags + always on bits
            }
        }
...
!macro set_zx arg1, arg2      {        ;precharging zp,x & immediate status
            +load_flag arg2
            pha         ;use stack to load status
...
far_ret 
        ...
        +trap_cc
        +trap_vc
        cmp #('F' xor $aa)  ;returned registers OK?
        +trap_ne
        cpx #('A'+1)
        +trap_ne
        cpy #('R'-3)
...
jsr_ret = *-1           ;last address of jsr = return address
        ...
        +trap_cc
        +trap_vc
        cmp #('J' xor $aa)  ;returned registers OK?
        +trap_ne
        cpx #('S'+1)
        +trap_ne
        cpy #('R'-6)
...
brk_ret                 ;address of break return
        ...
        dey
        dey
        cmp #('B' xor $aa)  ;returned registers OK?
        +trap_ne
        cpx #('R'+1)
        +trap_ne
        cpy #('K'-6)
...
tand    lda zpAN,x
        sta tandi1
        +set_ax  absANa,0
tandi1  = *+1     ;target for immediate operand
        and #99
...
tand1   lda zpAN,x
        sta tandi2
        +set_ax  absANa,$ff
tandi2  = *+1     ;target for immediate operand
        and #99
...
teor    lda zpEO,x
        sta teori1
        +set_ax  absEOa,0
teori1  = *+1     ;target for immediate operand
        eor #99
        +tst_ax  absrlo,absflo,0
        dex
        bpl teor
...
teor1   lda zpEO,x
        sta teori2
        +set_ax  absEOa,$ff
teori2  = *+1     ;target for immediate operand
        eor #99
        +tst_ax  absrlo,absflo,$ff-fnz
        dex
        bpl teor1
...
tora    lda zpOR,x
        sta torai1
        +set_ax  absORa,0
torai1  = *+1     ;target for immediate operand
        ora #99
        +tst_ax  absrlo,absflo,0
        dex
        bpl tora
...
tora1   lda zpOR,x
        sta torai2
        +set_ax  absORa,$ff
torai2  = *+1     ;target for immediate operand
        ora #99
        +tst_ax  absrlo,absflo,$ff-fnz
        dex
        bpl tora1
