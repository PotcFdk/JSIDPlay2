...
public class LameUI extends JFrame {
							...
							new PropertyChangeListener() {
								...
								public void propertyChange(
									...
									if ("progress".equals(evt.getPropertyName())) {
										bar.setValue(Integer.valueOf(evt
	...
	protected MouseAdapter setPicture = new MouseAdapter() {
		...
		public void mouseClicked(MouseEvent e) {
			...
			final int result = fc.showOpenDialog(LameUI.this);
			if (result == JFileChooser.APPROVE_OPTION
					&& fc.getSelectedFile() != null) {
				try {
					pictureFilename = fc.getSelectedFile().getAbsolutePath();
					picture.setComposerImage(new ImageIcon(fc.getSelectedFile()
	...
	public Action doSaveTags = new AbstractAction() {

		@Override
		public void actionPerformed(ActionEvent e) {
			DefaultTableModel model = (DefaultTableModel) files.getModel();
			for (int row = 0; row < model.getRowCount(); row++) {
				final String type = String.valueOf(model.getValueAt(row, 1));
				final String filename = String
						.valueOf(model.getValueAt(row, 2));
				final JProgressBar bar = (JProgressBar) model
						.getValueAt(row, 3);
				if ("wav".equals(type) && bar.getValue() < 100) {
					continue;
				}
				final String newFilename = filename.substring(0,
						...
						+ ".mp3";
				try {
					final File tmp = File.createTempFile("jump3r", ".mp3");
					final ArrayList<String> cmd = getSaveTagsCommand(filename,
							tmp);
					for (int i = 0; i < cmd.size(); i++) {
						System.out.print(cmd.get(i) + " ");
					}
					...
					new Thread(new Runnable() {
						...
						public void run() {
							...
							try {
								main.run(cmd.toArray(new String[cmd.size()]));
								if (!tmp.renameTo(new File(newFilename))) {
									System.err.println("Cannot rename to "
											+ newFilename);
								}
							} catch (IOException e) {
								e.printStackTrace();
	...
	public LameUI() {
		try {
			...
			swix.insert(LameUI.class.getResource("LameUI.xml"), this);

			picture.addMouseListener(setPicture);
			...
			files.setDefaultRenderer(JComponent.class,
					new JComponentCellRenderer());
			files.getSelectionModel().addListSelectionListener(
					new ListSelectionListener() {

						@Override
						public void valueChanged(ListSelectionEvent e) {
							saveTags.setEnabled(files.getSelectedRowCount() > 0);
						}
					});
			final URL resource = LameUI.class.getResource("picture.png");
	...
	protected ArrayList<String> getSaveTagsCommand(final String filename,
			final File tmp) throws IOException {
		ArrayList<String> cmd = new ArrayList<String>();
		cmd.add("--space-id3v1");
		if (title.getSelectedItem() != null) {
			cmd.add("--tt");
			cmd.add(title.getSelectedItem().toString());
		}
		if (artist.getSelectedItem() != null) {
			cmd.add("--ta");
			cmd.add(artist.getSelectedItem().toString());
		}
		if (album.getSelectedItem() != null) {
			cmd.add("--tl");
			cmd.add(album.getSelectedItem().toString());
		}
		if (year.getSelectedItem() != null) {
			cmd.add("--ty");
			cmd.add(year.getSelectedItem().toString());
		}
		if (track.getSelectedItem() != null) {
			cmd.add("--tn");
			cmd.add(track.getSelectedItem().toString());
		}
		if (genre.getSelectedItem().toString().length() > 0) {
			cmd.add("--tg");
			cmd.add(genre.getSelectedItem().toString());
		}
		if (comment.getSelectedItem() != null) {
			cmd.add("--tc");
			cmd.add(comment.getSelectedItem().toString());
		}
